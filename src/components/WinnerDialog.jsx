...
export default function WinnerDialog({ game, onNewRound, onClose }) {
  ...
  const description = game.locker
    ? 'Die Runde wurde im Verlauf gespeichert, zählt aber nicht in der Bestenliste.'
    : 'Die Runde wurde gespeichert und wird mit Railway synchronisiert.'
  return <Dialog title={winners.length > 1 ? 'Unentschieden!' : `${winners[0].name} gewinnt!`} description={description} onClose={onClose} className="winner-dialog">
